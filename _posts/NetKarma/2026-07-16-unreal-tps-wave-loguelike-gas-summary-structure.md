...
- Subsystem이 `Initialize`될 때 **한 번에 초기화**한다.
...
특정 행동이 트리거되는 경우는 **GA로 `Event.Trigger`를 델리게이트로 받아 `ActivateAbility`** 한다. 트리거 후 GE를 떼어내야 하는 경우에만 **`EndAbility`에서 명시적으로 제거**한다.
...
GE는 조건이 필요 없기 때문에, 증강을 선택하는 즉시 **`EffectToActor`로 바로 붙여준다.** 트리거를 기다릴 필요가 없는 패시브 증강은 이 경로다.
