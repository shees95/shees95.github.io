...
[스페셜 탄 리스트]({% post_url NetKarma/2026-07-04-unreal-tps-wave-loguelike-weapon-system-3 %})에 이어서, 탄 배열 순서를 다루는 기능을 추가했다.
...
[히트 판정]({% post_url NetKarma/2026-06-24-unreal-tps-wave-loguelike-til-hitresult %})에서 얻은 결과를 그냥 흩뿌리지 않고, 데미지·약점 여부·적용된 GE를 한 묶음으로 만들어 `GA_Fire`가 필요한 시점에 받아 쓰게 했다. [증강 시스템의 이벤트 트리거]({% post_url NetKarma/2026-07-11-unreal-tps-wave-loguelike-augment-system-2 %})도 결국 이 정보를 근거로 발동하는 것이라, 여기서 정보가 깔끔하게 정리돼야 뒷단이 편해진다.
