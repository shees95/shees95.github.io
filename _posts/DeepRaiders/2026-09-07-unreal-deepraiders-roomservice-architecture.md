...
> 9월 7일~11일 주간 작업을 주제별로 나눈 회고다. 게시 날짜는 실제 구현일과 다를 수 있다. 코드와 구현 상태는 이번 작업에서 정리한 소스 발췌와 실행 기록을 기준으로 한다.
...
<!-- 캡처: Title의 방 목록 화면과 Master·GameServer 프로세스가 구분되는 실행 화면. 토큰과 비밀값은 제외. -->
