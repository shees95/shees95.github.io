...
약 `18ms`가 걸리던 특정 작업이 `1.8ms` 수준까지 감소했다.
...
각 위치의 값을 독립적으로 계산할 수 있는 구간은 병렬 처리에 적합하다고 생각했다.

CPU에서 긴 작업 하나로 처리하면 해당 프레임의 Game Thread나 작업 스레드 대기가 길어지니,
같은 계산을 GPU 병렬 연산으로 전환하면 다수의 복셀 값을 동시에 처리해 전체 소요 시간을 줄일 수 있다고 판단했다.
...
복셀을 계속 생성하는 상황에서 평균 100 FPS대까지 유지되는 모습을 확인했다.
...
처음에는 멀티플레이 테스트 중 발생한 현상이라는 이유로 네트워크를 의심했다.
